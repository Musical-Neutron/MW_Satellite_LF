...
This package can be installed by cloning it from the github repository:

```bash
    git clone git@github.com:Musical-Neutron/MW_Satellite_LF.git
    cd MW_Satellite_LF
    pip install .
```
...
## 4.0 Satellite data file structure

The data file for all known MW satellite galaxies (including those which have
not yet been spectroscopically confirmed) is provided in
[`home/Input_Data/All_satellites.csv`](/Input_Data/All_satellites.csv).
...
## 5.0 Survey data file structure

The data files for all surveys that can be used in the analysis are provided in
[`home/Input_Data/Surveys.csv`](/Input_Data/Surveys.csv).
Each column corresponds to a particular survey and response function.
By default, this code ships with three:
...
## 6.0 Ouput data file structure

The [`home/Output_Data`](/Output_Data) directory ships empty but can be filled
with outputs from [compute_satellites.py](/compute_satellites.py). By default
the accompanying [script](/script_compute_satellites.py) will write files to
this location. They will be in the following format:
...
The groups correspond to those described in Section 3.0.
