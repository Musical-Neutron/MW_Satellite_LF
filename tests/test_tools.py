...
class CartesianToSphericalTests(unittest.TestCase):
    ...
    def test_cartesian_to_spherical_validation(self):
        ...
        self.assertRaises(ValueError, cartesian_to_spherical, [1.0, 2.5, 3.0, 4.5])
        self.assertRaises(
            ValueError, cartesian_to_spherical, [[1.0, 2.5, 3.0, 4.5], [5.0, 6.5]]
        )
    ...
    def test_cartesian_to_spherical_function(self):
        ...
        self.assertIsNone(
            np.testing.assert_array_almost_equal(sph_vec, self.expected_sph_vector)
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(
                sph_vec_set, self.expected_sph_vector_set
            )
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(sph_vec_list, self.expected_sph_vector)
        )
        self.assertIsNone(
            np.testing.assert_array_almost_equal(
                sph_vec_set_list, self.expected_sph_vector_set
            )
        )
